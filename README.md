...
eco-knock-fe는 동아리 서비스 **ECO-KNOCK**의 모바일 중심 프론트엔드입니다. React와 Vite를 기반으로 인증, 모아두기, 동방 환경 현황, 그룹, 마이페이지, KRT 코인 및 AI 채팅 기능을 구현하고 있습니다.
...
- 환경 항목별 최근 12개 과거 시계열 상세 화면과 기본 시간 간격 조회·저장
...
- 내 소속 그룹과 전체 그룹 목록 조회, 모집 마감 제외 및 이름·최신순·마감 임박순 정렬
- 그룹 생성과 상세 조회, 서버가 계산한 권한에 따른 설정·수정·삭제·지원 버튼 노출
- 그룹 소개·정원·모집 방식·모집기간 인라인 수정과 그룹 이름 수정
- 그룹원 목록 조회, 그룹장 권한의 멤버 제거 및 그룹장 위임
- 그룹 지원서 작성·제출, 지원서 목록·상세 조회와 그룹장의 수락·거절 처리
- 현재 화면 위에서 열고 드래그해 닫을 수 있는 AI 채팅 오버레이
- 질문을 JSON으로 전송하고 `result.answer`를 표시하는 AI 채팅 API 연동
- 채팅 오버레이 최초 오픈 시 최근 대화 20쌍을 조회해 시간순으로 표시
...
| `/groups` | 그룹 | 내 소속·둘러보기 목록과 필터·정렬 API 연결 |
| `/groups/create` | 그룹 만들기 | 그룹 생성 API 연결 |
| `/groups/:groupId` | 그룹 상세 | 상세·권한·지원서 조회 및 그룹 정보 수정 API 연결 |
| `/groups/:groupId/apply` | 그룹 지원 | 지원서 작성·제출 API 연결 |
| `/groups/:groupId/settings` | 그룹 설정 | 권한별 설정 메뉴와 그룹 삭제 API 연결 |
| `/groups/:groupId/settings/name` | 그룹 이름 수정 | 그룹 이름 수정 API 연결 |
| `/groups/:groupId/settings/members` | 그룹 멤버 | 멤버 조회·제거·그룹장 위임 API 연결 |
...
| 동방 환경 | 현재 구간·과거 시계열·기본 시간 간격 조회 및 저장 |
| 회원 | 내 프로필 조회 |
| 그룹 | 내 그룹·전체 그룹·상세·멤버·지원서 조회, 그룹 생성·수정·삭제, 멤버 제거·그룹장 위임, 지원서 제출·수락·거절 |
| KRT 코인 | 활성 지갑 잔액, 지갑 순위, 내 보상 내역 조회 |
| AI 채팅 | 텍스트 질문 전송 및 답변 조회, 최근 채팅 기록 조회 |
