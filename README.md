...
eco-knock-fe는 React와 Vite 기반으로 개발 중인 ECO-KNOCK 프론트엔드 저장소입니다. 모바일 화면을 기준으로 공통 페이지 레이아웃, 모아두기, 마이페이지, 코인 상세, AI 채팅 오버레이 UI를 구현하고 있습니다.
...
- 기본 화면의 `MainPageFrame`과 코인 상세 화면의 `DetailPageFrame`
...
마이페이지와 코인 상세·로그는 외부에서 데이터가 전달되지 않으면 MOCK fallback 데이터를 표시하는 UI 단계입니다. 모아두기의 추가·삭제·순서 변경·복원 결과와 채팅 입력·전송, 첨부 버튼, 계정 관리 작업도 백엔드와 연결되지 않은 UI 단계입니다.
...
- `features`: 기능별 UI와 상호작용을 관리하며, 현재 `chat`, `coin`, `collection`, `mypage`가 있습니다.
