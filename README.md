...
eco-knock-fe는 React와 Vite 기반으로 개발 중인 ECO-KNOCK 프론트엔드 저장소입니다. 모바일 화면을 기준으로 공통 레이아웃, 모아두기, 마이페이지와 AI 채팅 오버레이를 구현하고 있습니다.
...
- 정적 사용자 데이터 기반 마이페이지 프로필과 코인 카드
- 하단 내비게이션이 없는 코인 상세 및 설정 UI
- Vite PWA 기본 설정

`/`, `/collection`, `/room`, `/recruit`, `/mypage`, `/mypage/coin`, `/mypage/settings` 라우트가 정의되어 있습니다. 동방과 부서모집 경로는 현재 임시로 `CollectionPage`를 사용합니다. 하단 Chat 버튼은 별도 경로로 이동하지 않고 기존 화면 위에 채팅 오버레이를 표시합니다.

현재 모아두기, 사용자와 코인 데이터는 정적 값이며, 채팅 입력·전송, 첨부 버튼과 설정 작업도 백엔드와 연결되지 않은 UI 단계입니다.
...
- 사용자 프로필과 코인 내역 조회 API 연동
- 로그아웃·회원 탈퇴 API, 확인 절차와 성공·오류 처리
- 동방과 부서모집의 별도 라우트 화면
